namespace PhotoshopFile
{
    ...
    public class Layer
    {
        ...
        public bool IsImageLayer
        {
            get
            {
                return (Rect.width > 0 && Children.Count == 0 && !IsTextLayer);
            }
        }

        public bool IsFolderLayer
        {
            get
            {
                return (Rect.width == 0 || Children.Count > 0);
            }
        }
        /// <summary>
        ...
        {
            IsTextLayer = true;
            //using (StreamReader sr = new StreamReader(dataReader.BaseStream))
            //{
            ...
            FontSize = dataReader.ReadFloat() - 5;
        ...
        public Layer GetImageChild(string tag = null)
        {
            foreach (Layer child in Children)
            {
                if (child.IsImageLayer)
                {
                    ...
                    {
                        if (child.Name.IndexOf(tag, System.StringComparison.OrdinalIgnoreCase) >= 0)
                        {
                            return child;
                        }
                    }
        ...
        public Layer GetFolderChild(string tag = null)
        {
            foreach (Layer child in Children)
            {
        ...
        public Layer GetTextChild(string tag = null)
        {
            foreach (Layer child in Children)
            {
                if (child.IsTextLayer)
        ...
        public Layer GetChildByTag(string tag)
        {
            foreach (Layer child in Children)
            {
                if (child.Name.IndexOf(tag, System.StringComparison.OrdinalIgnoreCase) >= 0)
