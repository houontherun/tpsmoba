...
public class BETween : MonoBehaviour {
	...
	void Update () {
		...
		if(ratio > 1.0f) {
			...
			else if(loopStyle == BETweenLoop.loop)
            {
                ratio -= 1.0f;
                timeAge -= duration;
            }
	...
	public void ApplyValue(Vector3 value) {
		if(type == BETweenType.position) {
			...
			else if(text != null) 			{ Color color = text.color;	color.a = value.x;	text.color = color; }
    ...
    public static BETween add(GameObject go, float _duration, BETweenType type) {
		...
		newTween.text = go.GetComponent<TMPro.TMP_Text>();
	...
	public static BETween alpha(GameObject go, float _duration, float _alpha) {
		...
		else if(tween.text != null) 		color = tween.text.color;
	...
	public static BETween size(GameObject go, float _duration, Vector2 _from, Vector2 _size) {
		BETween tween = add(go, _duration, BETweenType.size);
		tween.from = new Vector2(_from.x, _from.y);
		tween.to = new Vector2(_size.x, _size.y);
		tween.current = tween.from;
		tween.applyFrom = true;
		return tween;
	}
	public static BETween size(GameObject go, float _duration, Vector2 _size) {
        BETween tween = add(go, _duration, BETweenType.size);
        tween.from = tween.rt.sizeDelta;
        tween.to = new Vector2(_size.x, _size.y);
        tween.current = tween.from;
        tween.applyFrom = false;
        return tween;
